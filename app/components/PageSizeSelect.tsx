...
export default function PageSizeSelect({ value, onChange, className }: PageSizeSelectProps) {
  return (
    <label className={className ?? 'tms-toolbar tms-text-muted flex items-center gap-2'}>
      페이지당 표시
      <select
        value={String(value)}
        onChange={(event) => {
        ...
        className="tms-control rounded-lg border border-[#333] text-sm outline-none transition focus:border-indigo-400"
      >
