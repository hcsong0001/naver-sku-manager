...
export type NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationStatus =
...
export type NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationItemStatus =
...
export interface NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationItem {
  certificationItem: string;
  status: NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationItemStatus;
  meaning: string;
}

export interface NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationView {
  status: 'NAVER_READ_ONLY_FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFICATION_READY';
  panelTitle: string;
  description: string;
  taskId: 306;
  taskName: 'Naver Read-Only Final Execution Approval Review Outcome Certification Screen Flow';
  currentTaskNumber: 306;
  referenceTaskNumbers: readonly [305, 304, 303, 302, 301, 300, 299, 298, 297, 296, 293, 289, 276];
  ...
  sourceFinalExecutionApprovalReviewStatus: NaverReadOnlyFinalExecutionApprovalReviewStatus;
  ...
  certificationItems: NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationItem[];
  outcomeCertifiedReady: boolean;
  outcomeCertifiedPartialReady: boolean;
  outcomeCertificationBlocked: boolean;
  requiresSeparateTask307Approval: true;
...
const ALLOWED_REVIEW_STATUSES: NaverReadOnlyFinalExecutionApprovalReviewStatus[] = [
...
export const FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFICATION_STATUS_BY_REVIEW_STATUS: Record<
  NaverReadOnlyFinalExecutionApprovalReviewStatus,
...
export const NEXT_TASK_307_APPROVAL_PHRASE =
  'Task 307에서 Naver read-only 최종 실행 승인 안전 감사 Seal을 승인합니다. 이 단계는 실제 실행 승인이나 실제 실행이 아니라 최종 실행 승인 검토 결과 인증 이후의 안전 조건을 read-only로 봉인하는 단계입니다. Token 재발급·상품 조회 API 재호출·상품 수정·가격 변경·재고 변경·DB write·Worker 실행·Queue enqueue·Adapter 연결은 절대 수행하지 마세요. Token/Auth/Signature/Authorization 값과 raw API response는 출력하지 마세요.';
...
function buildCertificationSummary(input: {
  ...
  if (input.outcomeCertifiedReady) {
    return {
      certificationSummary:
        'Task 305 검토 결과(COMPLETE)를 read-only로 인증했습니다. 다음 안전 감사 Seal 단계 후보로 넘길 수 있는 상태입니다.',
      blockingReason: null,
    };
  }

  if (input.outcomeCertifiedPartialReady) {
    return {
      certificationSummary:
        'Task 305 검토 결과(PARTIAL)를 read-only로 인증했습니다. missing field notice를 유지한 채 다음 안전 감사 Seal 단계 후보로 넘길 수 있는 상태입니다.',
      blockingReason: null,
    };
  }
  ...
  const reasonMap: Record<
    ...
    string | null
  > = {
    NAVER_READ_ONLY_FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFIED_READY: null,
    NAVER_READ_ONLY_FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFIED_PARTIAL_READY:
      null,
    NAVER_READ_ONLY_FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_BLOCKED_BY_GW_IP:
  ...
  return {
    certificationSummary:
      'Task 305 검토 결과가 차단 상태이므로, 인증 결과는 원인별 보정 필요 상태만 표시합니다.',
    blockingReason:
...
function buildCertificationItems(input: {
  ...
  certificationStatus: NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationStatus;
}): NaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationItem[] {
  return [
    {
      certificationItem: 'Task 306 Final Execution Approval Review Outcome Certification',
      status: 'FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFICATION_READY',
      meaning: `Task 306 read-only 인증 패널을 구성했습니다. 현재 인증 상태는 ${input.certificationStatus}입니다.`,
    },
    {
      certificationItem: 'Task 305 Final Execution Approval Review',
      status: 'FINAL_EXECUTION_APPROVAL_REVIEW_CONFIRMED',
      meaning: `Task 305 검토 상태(${input.reviewStatus})를 확인했습니다.`,
    },
    {
    ...
    },
    {
      ...
      status: 'FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_STATUS_RECORDED',
      meaning: 'Task 305 검토 상태 7종을 Task 306 인증 상태 7종으로 1:1 매핑했습니다.',
    },
    {
      certificationItem: 'COMPLETE 인증 가능',
      status: 'CERTIFIED_READY_IF_COMPLETE',
      meaning: 'COMPLETE 계열이면 다음 안전 감사 Seal 단계 후보입니다.',
    },
    {
    ...
    {
      certificationItem: 'Task 307 승인 대기',
      status: 'PENDING_TASK_307_APPROVAL',
      meaning: 'Task 307은 사용자 별도 명시 승인 전까지 진행하지 않습니다.',
    },
    {
    ...
    },
    {
      ...
      meaning: 'Task 306은 최종 실행 승인 검토 결과를 read-only로 인증하는 단계입니다.',
    },
  ];
}

export function buildNaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationView(input: {
  ...
  const sourceFinalExecutionApprovalReviewStatus =
    input.finalExecutionApprovalReview.finalExecutionApprovalReviewStatus;

  if (!ALLOWED_REVIEW_STATUSES.includes(sourceFinalExecutionApprovalReviewStatus)) {
    throw new Error(
      `Unsupported final execution approval review status: ${sourceFinalExecutionApprovalReviewStatus}`
    );
  }

  const finalExecutionApprovalReviewOutcomeCertificationStatus =
    FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFICATION_STATUS_BY_REVIEW_STATUS[
      sourceFinalExecutionApprovalReviewStatus
    ];
  ...
  const outcomeCertificationBlocked =
    !outcomeCertifiedReady && !outcomeCertifiedPartialReady;
  ...
  return {
    status: 'NAVER_READ_ONLY_FINAL_EXECUTION_APPROVAL_REVIEW_OUTCOME_CERTIFICATION_READY',
    panelTitle:
      'Task 306 - Read-Only Final Execution Approval Review Outcome Certification',
    description:
      'Task 306은 Task 305 검토 결과를 다음 단계로 넘길 수 있는지 read-only로 인증하는 단계입니다. 이 패널은 실제 최종 실행 승인, 실제 실행 승인, 실제 실행, 상품 변경 승인, 실행 버튼 추가, POST / submit action 추가, Worker / Queue / Adapter 연결이 아닙니다.',
    taskId: 306,
    taskName:
      'Naver Read-Only Final Execution Approval Review Outcome Certification Screen Flow',
    currentTaskNumber: 306,
    referenceTaskNumbers: [305, 304, 303, 302, 301, 300, 299, 298, 297, 296, 293, 289, 276],
    isBatchJobResultDisplayOnly: true,
    isNaverReadOnlyFinalExecutionApprovalReviewOutcomeCertificationReady: true,
    isNaverReadOnlyFinalExecutionApprovalReviewReady: true,
    isNaverReadOnlyFinalExecutionApprovalPacketReady: true,
    ...
    certificationItems: buildCertificationItems({
      reviewStatus: sourceFinalExecutionApprovalReviewStatus,
      certificationStatus:
        finalExecutionApprovalReviewOutcomeCertificationStatus,
    }),
    outcomeCertifiedReady,
    ...
    requiresSeparateTask307Approval: true,
    nextTaskApprovalPhrase: NEXT_TASK_307_APPROVAL_PHRASE,
    actualFinalExecutionApprovalGranted: false,
    actualExecutionApprovalGranted: false,
    actualExecutionStarted: false,
    executionButtonAdded: false,
