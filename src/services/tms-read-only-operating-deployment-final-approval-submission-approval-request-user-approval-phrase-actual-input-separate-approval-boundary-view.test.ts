...
describe('Task 403 - User Approval Phrase Actual Input Separate Approval Boundary', () => {
  const cases: Array<{
    ...
    expected: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryStatus;
  }> = [
    {
      input:
  ...
  it('Task 402 READY → Task 403 READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[0].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryStatus,
      cases[0].expected
    );
  });

  it('Task 402 PARTIAL_READY → Task 403 PARTIAL_READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[1].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryStatus,
      cases[1].expected
  ...
  it('Task 402 BLOCKED → Task 403 BLOCKED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[2].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryStatus,
      cases[2].expected
  ...
  it('Task 402 NOT_STARTED → Task 403 NOT_STARTED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[3].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryStatus,
      cases[3].expected
  ...
  it('14개 User Approval Phrase Actual Input Separate Approval Boundary 그룹이 모두 생성된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[0].input)
      );
    assert.ok(view.userApprovalPhraseActualInputSeparateApprovalBoundaryReadinessItems.length > 0, 'group 1');
    assert.ok(view.userApprovalPhraseFinalPreInputLockOutcomeCertificationReferenceSeparateApprovalBoundaryItems.length > 0, 'group 2');
    assert.ok(view.userApprovalPhraseActualInputRequiresSeparateApprovalBoundaryItems.length > 0, 'group 3');
    assert.ok(view.userApprovalPhraseExampleDisplaySeparateApprovalBoundaryItems.length > 0, 'group 4');
    assert.ok(view.userApprovalPhraseInputUiNonAdditionSeparateApprovalBoundaryItems.length > 0, 'group 5');
    assert.ok(view.userApprovalPhraseActualInputNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 6');
    assert.ok(view.userApprovalPhraseSubmissionNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 7');
    assert.ok(view.userApprovalAcceptanceNonGrantSeparateApprovalBoundaryItems.length > 0, 'group 8');
    assert.ok(view.explicitApprovalRequestCreationNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 9');
    assert.ok(view.explicitApprovalRequestSubmissionNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 10');
    assert.ok(view.finalApprovalSubmissionNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 11');
    assert.ok(view.deploymentOperatingTransitionNonExecutionSeparateApprovalBoundaryItems.length > 0, 'group 12');
    assert.ok(view.operatingDbRuntimeWorkerQueueAdapterSeparateApprovalBoundaryItems.length > 0, 'group 13');
    assert.ok(view.apiSecretUiActionPostSeparateApprovalBoundaryItems.length > 0, 'group 14');
  });

  it('승인 문구 예시와 별도 승인 경계 플래그가 read-only로 유지된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseActualInputSeparateApprovalBoundaryView(
        makeDummy402View(cases[0].input)
      );
    ...
    assert.match(view.userApprovalPhraseGuidance, /별도 승인/);
    ...
    assert.equal(view.actualUserApprovalPhraseActualInputSeparateApprovalBoundarySubmitted, false);
    assert.equal(view.actualUserApprovalPhraseActualInputSeparateApprovalGranted, false);
  ...
  it('추천값들이 정확하다', () => {
    ...
    assert.equal(
      view.recommendedActualInputSeparateApprovalBoundaryDecision,
  ...
  it('입력 ViewModel의 Task 402 결과를 변경하지 않는다', () => {
    const input = makeDummy402View(cases[0].input);
    const sourceStatusBefore =
      input.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus;
