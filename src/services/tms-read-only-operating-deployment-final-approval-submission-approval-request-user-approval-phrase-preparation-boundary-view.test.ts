...
function makeDummy394Item(outcomeCertificationItemId: string) {
  return {
    outcomeCertificationItemId,
    sourceLockReviewItemId: `source-${outcomeCertificationItemId}`,
    category: 'EXPLICIT_APPROVAL_REQUEST_SUBMISSION_LOCK_OUTCOME_CERTIFICATION_READINESS' as const,
    label: `Task 394 ${outcomeCertificationItemId}`,
    description: `Task 394 description ${outcomeCertificationItemId}`,
    sourceLockReviewStatus:
      'EXPLICIT_APPROVAL_REQUEST_SUBMISSION_LOCK_REVIEW_READY' as const,
...
function makeDummy394View(
  status: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestExplicitApprovalRequestSubmissionLockOutcomeCertificationStatus
  ...
  const item = makeDummy394Item('task394-item-1');
  const outcomeCertificationItems = Array.from({ length: 14 }, (_, idx) =>
    makeDummy394Item(`task394-item-${idx + 1}`)
  );
  ...
  return {
    ...
    explicitApprovalRequestSubmissionLockOutcomeCertificationReadinessItems: [item],
    explicitApprovalRequestSubmissionLockReviewReferenceOutcomeCertificationItems: [item],
    explicitApprovalPhraseNonInputLockOutcomeCertificationItems: [item],
    explicitApprovalPhraseNonSubmissionLockOutcomeCertificationItems: [item],
    explicitApprovalAcceptanceNonGrantLockOutcomeCertificationItems: [item],
    explicitApprovalRequestCreationLockOutcomeCertificationItems: [item],
    explicitApprovalRequestSubmissionLockOutcomeCertificationItems: [item],
    approvalRequestCreationNonExecutionLockOutcomeCertificationItems: [item],
    approvalRequestSubmissionNonExecutionLockOutcomeCertificationItems: [item],
    ...
    explicitApprovalRequestSubmissionLockOutcomeCertificationReadinessItemCount: 1,
    explicitApprovalRequestSubmissionLockReviewReferenceOutcomeCertificationItemCount: 1,
    explicitApprovalPhraseNonInputLockOutcomeCertificationItemCount: 1,
    explicitApprovalPhraseNonSubmissionLockOutcomeCertificationItemCount: 1,
    explicitApprovalAcceptanceNonGrantLockOutcomeCertificationItemCount: 1,
    explicitApprovalRequestCreationLockOutcomeCertificationItemCount: 1,
    explicitApprovalRequestSubmissionLockOutcomeCertificationItemCount: 1,
    approvalRequestCreationNonExecutionLockOutcomeCertificationItemCount: 1,
    approvalRequestSubmissionNonExecutionLockOutcomeCertificationItemCount: 1,
...
describe('Task 395 - User Approval Phrase Preparation Boundary', () => {
  const cases: Array<{
    input: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestExplicitApprovalRequestSubmissionLockOutcomeCertificationStatus;
    expected: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryStatus;
  }> = [
    {
      input:
  ...
  it('Task 394 READY → Task 395 READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[0].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryStatus,
      cases[0].expected
    );
  });

  it('Task 394 PARTIAL_READY → Task 395 PARTIAL_READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[1].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryStatus,
      cases[1].expected
  ...
  it('Task 394 BLOCKED → Task 395 BLOCKED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[2].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryStatus,
      cases[2].expected
  ...
  it('Task 394 NOT_STARTED → Task 395 NOT_STARTED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[3].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryStatus,
      cases[3].expected
  ...
  it('14개 User Approval Phrase Preparation Boundary 그룹이 모두 생성된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[0].input)
      );
    assert.ok(view.userApprovalPhrasePreparationBoundaryReadinessItems.length > 0, 'group 1');
    assert.ok(view.submissionLockOutcomeCertificationReferencePreparationBoundaryItems.length > 0, 'group 2');
    assert.ok(view.userApprovalPhraseRequiredPreparationBoundaryItems.length > 0, 'group 3');
    assert.ok(view.userApprovalPhraseExampleDisplayPreparationBoundaryItems.length > 0, 'group 4');
    assert.ok(view.userApprovalPhraseNonInputPreparationBoundaryItems.length > 0, 'group 5');
    assert.ok(view.userApprovalPhraseNonSubmissionPreparationBoundaryItems.length > 0, 'group 6');
    assert.ok(view.userApprovalAcceptanceNonGrantPreparationBoundaryItems.length > 0, 'group 7');
    assert.ok(view.explicitApprovalRequestCreationNonExecutionPreparationBoundaryItems.length > 0, 'group 8');
    assert.ok(view.explicitApprovalRequestSubmissionNonExecutionPreparationBoundaryItems.length > 0, 'group 9');
    assert.ok(view.finalApprovalSubmissionNonExecutionPreparationBoundaryItems.length > 0, 'group 10');
    assert.ok(view.finalApprovalGrantNonExecutionPreparationBoundaryItems.length > 0, 'group 11');
    assert.ok(view.deploymentOperatingTransitionNonExecutionPreparationBoundaryItems.length > 0, 'group 12');
    assert.ok(view.operatingDbRuntimeWorkerQueueAdapterPreparationBoundaryItems.length > 0, 'group 13');
    assert.ok(view.apiSecretUiActionPostPreparationBoundaryItems.length > 0, 'group 14');
  });

  it('승인 문구 준비 경계 플래그와 예시가 read-only로 유지된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhrasePreparationBoundaryView(
        makeDummy394View(cases[0].input)
      );
    ...
    assert.match(view.userApprovalPhraseGuidance, /입력\/제출\/수락/);
  ...
  it('추천값들이 정확하다', () => {
    ...
    assert.equal(
      view.recommendedUserApprovalPhrasePreparationBoundaryDecision,
    ...
    assert.equal(
      ...
      'USER_APPROVAL_PHRASE_PREPARATION_BOUNDARY_ONLY'
    );
  ...
  it('입력 ViewModel의 Task 394 결과를 변경하지 않는다', () => {
    const input = makeDummy394View(cases[0].input);
    const sourceStatusBefore =
      input.operatingDeploymentFinalApprovalSubmissionApprovalRequestExplicitApprovalRequestSubmissionLockOutcomeCertificationStatus;
