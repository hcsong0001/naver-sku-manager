...
const SERVICE_FILE = resolve(
  __dirname,
  'sku-keyword-final-approval-execution-naver-api-token-first-test-separate-approval-final-hold-non-release-handoff-closure-final-status-seal-confirmation-final-review-closure-status-final-closure-boundary-view.service.ts'
);
...
describe('Task 106 View Model 기본 필드', () => {
  ...
  it('statusLabel이 FINAL CLOSURE BOUNDARY를 포함함', () => {
    assert.ok(view.statusLabel.includes('FINAL CLOSURE BOUNDARY'));
  });
  ...
  it('finalNotice가 Task 106 이후에도 전환 불가를 표현함', () => {
    assert.ok(view.finalNotice.includes('Task 106'));
...
describe('Task 범위와 이전 커밋 참조', () => {
  ...
  it('taskRangeLabel이 Task 41~105 흐름을 표현함', () => {
    assert.ok(view.taskRangeLabel.includes('Task 41'));
    assert.ok(view.taskRangeLabel.includes('105'));
  });

  it('previousFinalClosureSummaryCommit이 Task 105 기준 커밋임', () => {
    assert.strictEqual(view.previousFinalClosureSummaryCommit, '48949de');
  });

  it('previousFinalClosureSummaryLabel이 Task 105를 참조함', () => {
    assert.ok(view.previousFinalClosureSummaryLabel.includes('Task 105'));
  });
});

describe('Final Closure Boundary Summary', () => {
  const items = buildView().finalClosureBoundarySummaryItems;
  ...
  it('폐쇄 요약과 실제 승인을 분리함', () => {
    assert.ok(items.some((item) => item.description.includes('분리')));
  });
});

describe('Final Closure Summary Is Not Release', () => {
  const items = buildView().finalClosureSummaryIsNotReleaseItems;
  ...
  it('보류 해제 완료가 아님을 표현함', () => {
    assert.ok(items.some((item) => item.description.includes('보류 해제')));
  });

  it('모든 항목이 notReleaseReason을 가짐', () => {
    for (const item of items) {
...
describe('Final Closure Review Is Not Approval', () => {
  const items = buildView().finalClosureReviewNotApprovalItems;
  ...
  it('검토와 승인 부여를 분리함', () => {
    assert.ok(items.some((item) => item.correctInterpretation.includes('승인') || item.description.includes('승인')));
  });

  it('실행 허용이 아님을 포함함', () => {
    assert.ok(items.some((item) => item.correctInterpretation.includes('실행 허용 아님')));
  });

  it('token 발급 허용이 아님을 포함함', () => {
    assert.ok(items.some((item) => item.correctInterpretation.includes('token 발급 허용 아님')));
  });
});
...
describe('Remaining Non-Release', () => {
  ...
  it('보류 해제 승인 미부여를 표현함', () => {
    assert.ok(items.some((item) => item.remainingState.includes('보류 해제 승인')));
  });

  it('실행 허용 미부여를 표현함', () => {
    assert.ok(items.some((item) => item.remainingState.includes('실행 허용')));
  });
...
describe('Required Before Any Future Transition', () => {
  ...
  it('Task 41~106 증거를 포함함', () => {
    assert.ok(items.some((item) => item.requiredEvidence.includes('Task 41~106')));
  });
...
describe('순수 함수와 참조 격리', () => {
  it('여러 호출이 동일한 핵심 상태를 반환함', () => {
    ...
    assert.strictEqual(first.previousFinalClosureSummaryCommit, second.previousFinalClosureSummaryCommit);
  });
  ...
  it('배열 참조가 호출마다 분리됨', () => {
    assert.notStrictEqual(buildView().finalClosureBoundarySummaryItems, buildView().finalClosureBoundarySummaryItems);
