...
function makeDummy401Item(finalPreInputLockReviewItemId: string) {
  return {
    finalPreInputLockReviewItemId,
    sourceOutcomeCertificationItemId: `source-${finalPreInputLockReviewItemId}`,
    category: 'USER_APPROVAL_PHRASE_FINAL_PRE_INPUT_LOCK_REVIEW_READINESS' as const,
    label: `Task 401 ${finalPreInputLockReviewItemId}`,
    description: `Task 401 description ${finalPreInputLockReviewItemId}`,
    sourceOutcomeCertificationStatus:
      'USER_APPROVAL_PHRASE_EXPLICIT_INPUT_BOUNDARY_OUTCOME_CERTIFIED_READY' as const,
...
function makeDummy401View(
  status: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus
): TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView {
  const item = makeDummy401Item('task401-item-1');
  const finalPreInputLockReviewItems = Array.from({ length: 14 }, (_, idx) =>
    makeDummy401Item(`task401-item-${idx + 1}`)
  );
  ...
  return {
    ...
    userApprovalPhraseFinalPreInputLockReviewReadinessItems: [item],
    userApprovalPhraseExplicitInputBoundaryOutcomeCertificationReferenceFinalPreInputLockReviewItems: [item],
    userApprovalPhraseRequiredFinalPreInputLockReviewItems: [item],
    userApprovalPhraseExampleDisplayFinalPreInputLockReviewItems: [item],
    userApprovalPhraseInputUiNonAdditionFinalPreInputLockReviewItems: [item],
    userApprovalPhraseInputNonExecutionFinalPreInputLockReviewItems: [item],
    userApprovalPhraseSubmissionNonExecutionFinalPreInputLockReviewItems: [item],
    userApprovalAcceptanceNonGrantFinalPreInputLockReviewItems: [item],
    explicitApprovalRequestCreationNonExecutionFinalPreInputLockReviewItems: [item],
    explicitApprovalRequestSubmissionNonExecutionFinalPreInputLockReviewItems: [item],
    finalApprovalSubmissionNonExecutionFinalPreInputLockReviewItems: [item],
    deploymentOperatingTransitionNonExecutionFinalPreInputLockReviewItems: [item],
    operatingDbRuntimeWorkerQueueAdapterFinalPreInputLockReviewItems: [item],
    apiSecretUiActionPostFinalPreInputLockReviewItems: [item],
    finalPreInputLockReviewSummaryCards: [],
    readyFinalPreInputLockReviewItems: finalPreInputLockReviewItems,
    partialReadyFinalPreInputLockReviewItems: [],
    blockedFinalPreInputLockReviewItems: [],
    notStartedFinalPreInputLockReviewItems: [],
    userApprovalPhraseFinalPreInputLockReviewReadinessItemCount: 1,
    userApprovalPhraseExplicitInputBoundaryOutcomeCertificationReferenceFinalPreInputLockReviewItemCount: 1,
    userApprovalPhraseRequiredFinalPreInputLockReviewItemCount: 1,
    userApprovalPhraseExampleDisplayFinalPreInputLockReviewItemCount: 1,
    userApprovalPhraseInputUiNonAdditionFinalPreInputLockReviewItemCount: 1,
    userApprovalPhraseInputNonExecutionFinalPreInputLockReviewItemCount: 1,
    userApprovalPhraseSubmissionNonExecutionFinalPreInputLockReviewItemCount: 1,
    userApprovalAcceptanceNonGrantFinalPreInputLockReviewItemCount: 1,
    explicitApprovalRequestCreationNonExecutionFinalPreInputLockReviewItemCount: 1,
    explicitApprovalRequestSubmissionNonExecutionFinalPreInputLockReviewItemCount: 1,
    finalApprovalSubmissionNonExecutionFinalPreInputLockReviewItemCount: 1,
    deploymentOperatingTransitionNonExecutionFinalPreInputLockReviewItemCount: 1,
    operatingDbRuntimeWorkerQueueAdapterFinalPreInputLockReviewItemCount: 1,
    apiSecretUiActionPostFinalPreInputLockReviewItemCount: 1,
    readyItemCount: finalPreInputLockReviewItems.length,
...
describe('Task 402 - User Approval Phrase Final Pre-Input Lock Outcome Certification', () => {
  const cases: Array<{
    input: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus;
    expected: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus;
  }> = [
    {
      input:
  ...
  it('Task 401 READY → Task 402 READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[0].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus,
      cases[0].expected
    );
  });

  it('Task 401 PARTIAL_READY → Task 402 PARTIAL_READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[1].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus,
      cases[1].expected
  ...
  it('Task 401 BLOCKED → Task 402 BLOCKED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[2].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus,
      cases[2].expected
  ...
  it('Task 401 NOT_STARTED → Task 402 NOT_STARTED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[3].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationStatus,
      cases[3].expected
  ...
  it('14개 User Approval Phrase Final Pre-Input Lock Outcome Certification 그룹이 모두 생성된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[0].input)
      );
    assert.ok(view.userApprovalPhraseFinalPreInputLockOutcomeCertificationReadinessItems.length > 0, 'group 1');
    assert.ok(view.userApprovalPhraseFinalPreInputLockReviewReferenceOutcomeCertificationItems.length > 0, 'group 2');
    assert.ok(view.userApprovalPhraseRequiredFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 3');
    assert.ok(view.userApprovalPhraseExampleDisplayFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 4');
    assert.ok(view.userApprovalPhraseInputUiNonAdditionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 5');
    assert.ok(view.userApprovalPhraseInputNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 6');
    assert.ok(view.userApprovalPhraseSubmissionNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 7');
    assert.ok(view.userApprovalAcceptanceNonGrantFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 8');
    assert.ok(view.explicitApprovalRequestCreationNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 9');
    assert.ok(view.explicitApprovalRequestSubmissionNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 10');
    assert.ok(view.finalApprovalSubmissionNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 11');
    assert.ok(view.deploymentOperatingTransitionNonExecutionFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 12');
    assert.ok(view.operatingDbRuntimeWorkerQueueAdapterFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 13');
    assert.ok(view.apiSecretUiActionPostFinalPreInputLockOutcomeCertificationItems.length > 0, 'group 14');
  });

  it('승인 문구 예시와 최종 Lock 결과 인증 플래그가 read-only로 유지된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockOutcomeCertificationView(
        makeDummy401View(cases[0].input)
      );
    ...
    assert.equal(view.actualUserApprovalPhraseFinalPreInputLockReviewSubmitted, false);
    assert.equal(view.actualUserApprovalPhraseFinalPreInputLockOutcomeCertificationSubmitted, false);
  ...
  it('입력 ViewModel의 Task 401 결과를 변경하지 않는다', () => {
    const input = makeDummy401View(cases[0].input);
    const sourceStatusBefore =
      input.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus;
