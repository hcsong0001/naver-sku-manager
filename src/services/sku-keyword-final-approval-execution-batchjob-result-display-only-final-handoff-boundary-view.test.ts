...
test('sku-keyword-final-approval-execution-batchjob-result-display-only-final-handoff-boundary-view.service', async (t) => {
  await t.test('should build batchjob execution result display only final handoff boundary view correctly', () => {
    const view = buildNaverApiTokenFirstTestSeparateApprovalFinalHoldNonReleaseHandoffClosureFinalStatusSealConfirmationFinalReviewClosureStatusFinalClosureFinalStatusExecutionBatchJobResultDisplayOnlyFinalHandoffBoundaryView(null);

    assert.equal(view.taskName, 'Task 186 - BatchJob Execution Result Display-Only Final Handoff Boundary Screen Flow');
    assert.equal(view.panelTitle, 'BatchJob Execution Result Display-Only Final Handoff Boundary');
    assert.equal(view.boundaryStatus, 'BATCHJOB_RESULT_DISPLAY_ONLY_FINAL_HANDOFF_BOUNDARY_ACTIVE');

    assert.equal(view.isReadOnly, true);
    assert.equal(view.isDisplayOnlyFinalHandoffBoundary, true);
    ...
    assert.equal(view.previousExecutionBatchJobResultDisplayOnlyFinalAcceptanceHandoffCommit, '2854093');

    assert.equal(view.finalHandoffBoundaryItems.length, 1);
    ...
    assert.ok(view.finalNotice.includes('final acceptance handoff는 실행 허가가 아니며'));
    assert.ok(view.finalNotice.includes('display-only 상태 표시 boundary입니다'));
  });
});
