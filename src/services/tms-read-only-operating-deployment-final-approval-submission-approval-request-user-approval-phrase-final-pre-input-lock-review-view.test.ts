...
function makeDummy400Item(outcomeCertificationItemId: string) {
  return {
    outcomeCertificationItemId,
    sourceExplicitInputBoundaryItemId: `source-${outcomeCertificationItemId}`,
    category:
      'USER_APPROVAL_PHRASE_EXPLICIT_INPUT_BOUNDARY_OUTCOME_CERTIFICATION_READINESS' as const,
    label: `Task 400 ${outcomeCertificationItemId}`,
    description: `Task 400 description ${outcomeCertificationItemId}`,
    sourceExplicitInputBoundaryStatus:
      'USER_APPROVAL_PHRASE_EXPLICIT_INPUT_BOUNDARY_READY' as const,
...
function makeDummy400View(
  status: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseExplicitInputBoundaryOutcomeCertificationStatus
  ...
  const item = makeDummy400Item('task400-item-1');
  const outcomeCertificationItems = Array.from({ length: 14 }, (_, idx) =>
    makeDummy400Item(`task400-item-${idx + 1}`)
  );
  ...
  return {
    ...
    userApprovalPhraseExplicitInputBoundaryOutcomeCertificationReadinessItems: [item],
    userApprovalPhraseExplicitInputBoundaryReferenceOutcomeCertificationItems: [item],
    userApprovalPhraseRequiredExplicitInputOutcomeCertificationItems: [item],
    userApprovalPhraseExampleDisplayExplicitInputOutcomeCertificationItems: [item],
    userApprovalPhraseInputUiNonAdditionOutcomeCertificationItems: [item],
    userApprovalPhraseInputNonExecutionOutcomeCertificationItems: [item],
    userApprovalPhraseSubmissionNonExecutionOutcomeCertificationItems: [item],
    userApprovalAcceptanceNonGrantExplicitInputOutcomeCertificationItems: [item],
    explicitApprovalRequestCreationNonExecutionExplicitInputOutcomeCertificationItems: [item],
    explicitApprovalRequestSubmissionNonExecutionExplicitInputOutcomeCertificationItems: [item],
    finalApprovalSubmissionNonExecutionExplicitInputOutcomeCertificationItems: [item],
    deploymentOperatingTransitionNonExecutionExplicitInputOutcomeCertificationItems: [item],
    operatingDbRuntimeWorkerQueueAdapterExplicitInputOutcomeCertificationItems: [item],
    apiSecretUiActionPostExplicitInputOutcomeCertificationItems: [item],
    ...
    userApprovalPhraseExplicitInputBoundaryOutcomeCertificationReadinessItemCount: 1,
    userApprovalPhraseExplicitInputBoundaryReferenceOutcomeCertificationItemCount: 1,
    userApprovalPhraseRequiredExplicitInputOutcomeCertificationItemCount: 1,
    userApprovalPhraseExampleDisplayExplicitInputOutcomeCertificationItemCount: 1,
    userApprovalPhraseInputUiNonAdditionOutcomeCertificationItemCount: 1,
    userApprovalPhraseInputNonExecutionOutcomeCertificationItemCount: 1,
    userApprovalPhraseSubmissionNonExecutionOutcomeCertificationItemCount: 1,
    userApprovalAcceptanceNonGrantExplicitInputOutcomeCertificationItemCount: 1,
    explicitApprovalRequestCreationNonExecutionExplicitInputOutcomeCertificationItemCount: 1,
    explicitApprovalRequestSubmissionNonExecutionExplicitInputOutcomeCertificationItemCount: 1,
    finalApprovalSubmissionNonExecutionExplicitInputOutcomeCertificationItemCount: 1,
    deploymentOperatingTransitionNonExecutionExplicitInputOutcomeCertificationItemCount: 1,
    operatingDbRuntimeWorkerQueueAdapterExplicitInputOutcomeCertificationItemCount: 1,
    apiSecretUiActionPostExplicitInputOutcomeCertificationItemCount: 1,
...
describe('Task 401 - User Approval Phrase Final Pre-Input Lock Review', () => {
  const cases: Array<{
    input: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseExplicitInputBoundaryOutcomeCertificationStatus;
    expected: TmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus;
  }> = [
    {
      input:
  ...
  it('Task 400 READY → Task 401 READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[0].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus,
      cases[0].expected
    );
  });

  it('Task 400 PARTIAL_READY → Task 401 PARTIAL_READY', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[1].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus,
      cases[1].expected
  ...
  it('Task 400 BLOCKED → Task 401 BLOCKED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[2].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus,
      cases[2].expected
  ...
  it('Task 400 NOT_STARTED → Task 401 NOT_STARTED', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[3].input)
      );
    assert.equal(
      view.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewStatus,
      cases[3].expected
  ...
  it('14개 User Approval Phrase Final Pre-Input Lock Review 그룹이 모두 생성된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[0].input)
      );
    assert.ok(view.userApprovalPhraseFinalPreInputLockReviewReadinessItems.length > 0, 'group 1');
    assert.ok(view.userApprovalPhraseExplicitInputBoundaryOutcomeCertificationReferenceFinalPreInputLockReviewItems.length > 0, 'group 2');
    assert.ok(view.userApprovalPhraseRequiredFinalPreInputLockReviewItems.length > 0, 'group 3');
    assert.ok(view.userApprovalPhraseExampleDisplayFinalPreInputLockReviewItems.length > 0, 'group 4');
    assert.ok(view.userApprovalPhraseInputUiNonAdditionFinalPreInputLockReviewItems.length > 0, 'group 5');
    assert.ok(view.userApprovalPhraseInputNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 6');
    assert.ok(view.userApprovalPhraseSubmissionNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 7');
    assert.ok(view.userApprovalAcceptanceNonGrantFinalPreInputLockReviewItems.length > 0, 'group 8');
    assert.ok(view.explicitApprovalRequestCreationNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 9');
    assert.ok(view.explicitApprovalRequestSubmissionNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 10');
    assert.ok(view.finalApprovalSubmissionNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 11');
    assert.ok(view.deploymentOperatingTransitionNonExecutionFinalPreInputLockReviewItems.length > 0, 'group 12');
    assert.ok(view.operatingDbRuntimeWorkerQueueAdapterFinalPreInputLockReviewItems.length > 0, 'group 13');
    assert.ok(view.apiSecretUiActionPostFinalPreInputLockReviewItems.length > 0, 'group 14');
  });

  it('승인 문구 예시와 최종 Lock 검토 플래그가 read-only로 유지된다', () => {
    const view =
      buildTmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseFinalPreInputLockReviewView(
        makeDummy400View(cases[0].input)
      );
    ...
    assert.match(view.userApprovalPhraseGuidance, /Final Pre-Input Lock Review/);
  ...
  it('추천값들이 정확하다', () => {
    ...
    assert.equal(
      view.recommendedFinalPreInputLockReviewDecision,
    ...
    assert.equal(
      ...
      'USER_APPROVAL_PHRASE_FINAL_PRE_INPUT_LOCK_REVIEW_ONLY'
    );
  ...
  it('입력 ViewModel의 Task 400 결과를 변경하지 않는다', () => {
    const input = makeDummy400View(cases[0].input);
    const sourceStatusBefore =
      input.operatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseExplicitInputBoundaryOutcomeCertificationStatus;
