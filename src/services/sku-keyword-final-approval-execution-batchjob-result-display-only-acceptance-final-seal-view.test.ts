...
import { buildNaverApiTokenFirstTestSeparateApprovalFinalHoldNonReleaseHandoffClosureFinalStatusSealConfirmationFinalReviewClosureStatusFinalClosureFinalStatusExecutionBatchJobResultDisplayOnlyAcceptanceFinalSealView } from './sku-keyword-final-approval-execution-batchjob-result-display-only-acceptance-final-seal-view.service';

test('sku-keyword-final-approval-execution-batchjob-result-display-only-acceptance-final-seal-view.service', async (t) => {
  await t.test('should build batchjob execution result display only acceptance final seal view correctly', () => {
    const view = buildNaverApiTokenFirstTestSeparateApprovalFinalHoldNonReleaseHandoffClosureFinalStatusSealConfirmationFinalReviewClosureStatusFinalClosureFinalStatusExecutionBatchJobResultDisplayOnlyAcceptanceFinalSealView(null);

    assert.equal(view.taskName, 'Task 181 - BatchJob Execution Result Display-Only Acceptance Final Seal Screen Flow');
    assert.equal(view.panelTitle, 'BatchJob Execution Result Display-Only Acceptance Final Seal');
    assert.equal(view.sealStatus, 'BATCHJOB_RESULT_DISPLAY_ONLY_ACCEPTANCE_FINAL_SEAL_ACTIVE');

    assert.equal(view.isReadOnly, true);
    assert.equal(view.isDisplayOnlyAcceptanceFinalSeal, true);
    ...
    assert.equal(view.previousExecutionBatchJobResultDisplayOnlyAcceptanceFinalEvidenceCommit, 'd0b3778');

    assert.equal(view.acceptanceFinalSealItems.length, 1);
    ...
    assert.ok(view.finalNotice.includes('acceptance final evidence는 실행 허가가 아니며'));
