...
import {
  NEXT_TASK_342_APPROVAL_PHRASE,
  TASK_342_NEXT_PHASE_RECOMMENDATION,
...
function buildTask340(statusOverride?: StatusOverride) {
  ...
  return buildTmsReadOnlyVpsDeploymentCandidateFinalClosureSummarySafetyAuditSealOutcomeCertificationView(
    {
      vpsDeploymentCandidateFinalClosureSummarySafetyAuditSeal:
        buildTask340(statusOverride),
    },
  );
}

test('Task 340 SEAL_READY → Task 341 CERTIFIED_READY', () => {
  const view = buildView();
  assert.equal(
    view.vpsDeploymentCandidateFinalClosureSummarySafetyAuditSealOutcomeCertificationStatus,
...
test('finalClosureSummarySafetySealOutcomeCertified true', () => {
  const view = buildView();
  assert.equal(view.finalClosureSummarySafetySealOutcomeCertified, true);
});

test('finalClosureSummarySafetySealItemsCertified true', () => {
  const view = buildView();
  assert.equal(view.finalClosureSummarySafetySealItemsCertified, true);
});

test('primaryClosureItemsSafetySealOutcomeCertified true', () => {
  const view = buildView();
  assert.equal(view.primaryClosureItemsSafetySealOutcomeCertified, true);
});

test('subsequentClosureItemsSafetySealOutcomeCertified true', () => {
  const view = buildView();
  assert.equal(view.subsequentClosureItemsSafetySealOutcomeCertified, true);
});
...
test('Task 342 승인 문구 포함', () => {
  const view = buildView();
  assert.ok(view.nextTaskApprovalPhrase.includes('Task 342'));
  assert.equal(view.nextTaskApprovalPhrase, NEXT_TASK_342_APPROVAL_PHRASE);
});

test('nextPhaseRecommendation에 운영 배포 설계 검토 전환 안내 포함', () => {
  const view = buildView();
  assert.ok(view.nextPhaseRecommendation.includes('운영 배포 설계 검토'));
  assert.equal(view.nextPhaseRecommendation, TASK_342_NEXT_PHASE_RECOMMENDATION);
});
