...
function buildSafetySeal(statusOverride?: StatusOverride) {
...
function buildView(statusOverride?: StatusOverride) {
  return buildTmsReadOnlyVpsDeploymentCandidateClosureSummarySafetyAuditSealOutcomeCertificationView(
    {
      vpsDeploymentCandidateClosureSummarySafetyAuditSeal: buildSafetySeal(statusOverride),
    },
  );
}

test('Task 336 SEAL_READY → Task 337 CERTIFIED_READY', () => {
  const view = buildView();
  assert.equal(
    view.vpsDeploymentCandidateClosureSummarySafetyAuditSealOutcomeCertificationStatus,
...
test('closureSummarySafetySealOutcomeCertified true', () => {
  const view = buildView();
  assert.equal(view.closureSummarySafetySealOutcomeCertified, true);
});
...
test('isReadOnlyVpsDeploymentCandidateClosureSummarySafetyAuditSealOutcomeCertification true', () => {
  const view = buildView();
  assert.equal(
    view.isReadOnlyVpsDeploymentCandidateClosureSummarySafetyAuditSealOutcomeCertification,
    true,
  );
});

test('requiresSeparateTask338Approval true', () => {
  const view = buildView();
  assert.equal(view.requiresSeparateTask338Approval, true);
});

test('Task 338 승인 문구 포함', () => {
  const view = buildView();
  assert.ok(view.nextTaskApprovalPhrase.includes('Task 338'));
  assert.equal(view.nextTaskApprovalPhrase, NEXT_TASK_338_APPROVAL_PHRASE);
});

test('outcomeCertificationItems가 12개 이상 생성되는지 확인', () => {
  const view = buildView();
