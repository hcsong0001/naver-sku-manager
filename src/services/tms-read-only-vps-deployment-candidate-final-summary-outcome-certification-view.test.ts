...
function buildFinalSummaryView(statusOverride?: StatusOverride) {
  ...
  return buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryView({
    vpsDeploymentCandidateDetailReview: detailReview,
    vpsDeploymentCandidateDetailReviewOutcomeCertification: detailReviewOutcomeCertification,
    vpsDeploymentCandidateSafetyAuditSeal: safetyAuditSeal,
    vpsDeploymentCandidateSafetyAuditSealOutcomeCertification: safetyAuditSealOutcomeCertification,
...
function buildInput(statusOverride?: StatusOverride) {
  return { vpsDeploymentCandidateFinalSummary: buildFinalSummaryView(statusOverride) };
}

test('Task 330 READY → Task 331 CERTIFIED_READY', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
  assert.equal(
    view.vpsDeploymentCandidateFinalSummaryOutcomeCertificationStatus,
...
test('outcomeCertificationItems 8개 생성 검증', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
...
test('totalOutcomeCertificationItemCount 8 검증', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
...
test('finalSummaryOutcomeCertified true', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
  assert.equal(view.finalSummaryOutcomeCertified, true);
});

test('finalSummaryItemsCertified true', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
  assert.equal(view.finalSummaryItemsCertified, true);
});
...
test('Task 332 승인 문구 포함', () => {
  const view = buildTmsReadOnlyVpsDeploymentCandidateFinalSummaryOutcomeCertificationView(
    buildInput(),
  );
  assert.equal(view.nextTaskApprovalPhrase, NEXT_TASK_332_APPROVAL_PHRASE);
});
