...
const SERVICE_FILE = resolve(
  __dirname,
  'sku-keyword-final-approval-execution-naver-api-token-first-test-separate-approval-final-hold-non-release-handoff-closure-final-status-seal-confirmation-final-review-closure-status-final-closure-final-status-summary-view.service.ts'
);
...
describe('Task 108 View Model 기본 필드', () => {
  ...
  it('statusLabel이 FINAL CLOSURE FINAL STATUS SUMMARY를 포함함', () => {
    assert.ok(view.statusLabel.includes('FINAL CLOSURE FINAL STATUS SUMMARY'));
  });
  ...
  it('finalNotice가 Task 108 이후에도 전환 불가를 표현함', () => {
    assert.ok(view.finalNotice.includes('Task 108'));
...
describe('Task 범위와 이전 커밋 참조', () => {
  ...
  it('taskRangeLabel이 Task 41~107 흐름을 표현함', () => {
    assert.ok(view.taskRangeLabel.includes('Task 41'));
    assert.ok(view.taskRangeLabel.includes('107'));
  });

  it('previousFinalClosureSealCommit이 Task 107 기준 커밋임', () => {
    assert.strictEqual(view.previousFinalClosureSealCommit, 'f473d2a');
  });

  it('previousFinalClosureSealLabel이 Task 107을 참조함', () => {
    assert.ok(view.previousFinalClosureSealLabel.includes('Task 107'));
  });
});

describe('Final Status Summary', () => {
  const items = buildView().finalStatusSummaryItems;
  ...
  it('최종 상태 요약 표현을 포함함', () => {
    assert.ok(items.some((item) => item.summaryState.includes('최종 상태')));
  });
});

describe('Final Closure Flow', () => {
  const items = buildView().finalClosureFlowItems;
  ...
  for (const keyword of ['Task 105', 'Task 106', 'Task 107', 'Task 108']) {
    it(`${keyword} 흐름을 포함함`, () => {
      assert.ok(items.some((item) => item.label.includes(keyword) || item.flowState.includes(keyword)));
    });
  }

  it('모든 항목이 flowState를 가짐', () => {
    for (const item of items) {
      assert.ok(item.flowState.length > 0);
    }
  });
...
describe('Non-Release Final Status', () => {
  const items = buildView().nonReleaseFinalStatusItems;
  ...
  it('보류 미해제 최종 상태를 표현함', () => {
    assert.ok(items.some((item) => item.nonReleaseState.includes('보류 해제')));
  });
...
describe('Not Release Approval', () => {
  ...
  it('최종 상태 요약과 승인 부여를 분리함', () => {
    assert.ok(items.some((item) => item.notApprovalReason.includes('아님')));
  });

  it('실행 허용 아님을 포함함', () => {
    assert.ok(items.some((item) => item.notApprovalReason.includes('실행 허용 아님')));
  });
...
describe('Required Before Any Future Transition', () => {
  ...
  it('Task 41~108 증거를 포함함', () => {
    assert.ok(items.some((item) => item.requiredEvidence.includes('Task 41~108')));
  });
...
describe('순수 함수와 참조 격리', () => {
  it('여러 호출이 동일한 핵심 상태를 반환함', () => {
    ...
    assert.strictEqual(first.previousFinalClosureSealCommit, second.previousFinalClosureSealCommit);
  });
  ...
  it('배열 참조가 호출마다 분리됨', () => {
    assert.notStrictEqual(buildView().finalStatusSummaryItems, buildView().finalStatusSummaryItems);
