...
function buildTask406View(
  ...
  return buildTmsFastConnectionNaverApiEnvExistenceNoSecretPreflightView(task405View);
}

describe('Task 407 - Fast Connection Naver Product Lookup One-Time Live Test Approval Packet', () => {
  const cases: Array<{
    ...
    expected: TmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus;
  }> = [
    {
      input:
      ...
      expected: 'TMS_FAST_CONNECTION_NAVER_PRODUCT_LOOKUP_ONE_TIME_LIVE_TEST_APPROVAL_PACKET_READY',
    },
    {
      input:
      ...
      expected: 'TMS_FAST_CONNECTION_NAVER_PRODUCT_LOOKUP_ONE_TIME_LIVE_TEST_APPROVAL_PACKET_PARTIAL_READY',
    },
    {
      input:
      ...
      expected: 'TMS_FAST_CONNECTION_NAVER_PRODUCT_LOOKUP_ONE_TIME_LIVE_TEST_APPROVAL_PACKET_BLOCKED',
    },
    {
      input:
      ...
      expected: 'TMS_FAST_CONNECTION_NAVER_PRODUCT_LOOKUP_ONE_TIME_LIVE_TEST_APPROVAL_PACKET_NOT_STARTED',
    },
  ];

  it('Task 406 READY 계열 → Task 407 READY', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[0].input)
    );
    assert.equal(view.fastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus, cases[0].expected);
  });

  it('Task 406 PARTIAL_READY 계열 → Task 407 PARTIAL_READY', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[1].input)
    );
    assert.equal(view.fastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus, cases[1].expected);
  });

  it('Task 406 BLOCKED 계열 → Task 407 BLOCKED', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[2].input)
    );
    assert.equal(view.fastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus, cases[2].expected);
  });

  it('Task 406 NOT_STARTED 계열 → Task 407 NOT_STARTED', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[3].input)
    );
    assert.equal(view.fastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus, cases[3].expected);
  });

  it('상태 매핑이 exhaustive하다 (4개 상태 모두 커버)', () => {
    for (const c of cases) {
      const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
        buildTask406View(c.input)
      );
      assert.equal(view.fastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketStatus, c.expected);
    }
    assert.equal(cases.length, 4);
  });

  it('8개 Live Test 승인 Packet 그룹이 생성된다', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[0].input)
    );
    assert.ok(view.liveTestApprovalPacketReadinessItems.length > 0, 'group 1');
    assert.ok(view.task406NoSecretPreflightReferenceItems.length > 0, 'group 2');
    assert.ok(view.oneTimeProductLookupPurposeItems.length > 0, 'group 3');
    assert.ok(view.oneTimeProductLookupTargetScopeItems.length > 0, 'group 4');
    assert.ok(view.liveTestApprovalPhraseGuidanceItems.length > 0, 'group 5');
    assert.ok(view.liveTestStillForbiddenActionsItems.length > 0, 'group 6');
    assert.ok(view.noEnvSecretTokenApiCallGuardItems.length > 0, 'group 7');
    assert.ok(view.nextFinalSafetyGateRoadmapItems.length > 0, 'group 8');
    assert.equal(view.liveTestApprovalPacketGroupCount, 8);
  });
  ...
  it('recommendedNextStep이 NAVER_PRODUCT_LOOKUP_ONE_TIME_FINAL_SAFETY_GATE다', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[0].input)
    );
    assert.equal(view.recommendedNextStep, 'NAVER_PRODUCT_LOOKUP_ONE_TIME_FINAL_SAFETY_GATE');
  });

  it('recommendedSafetyMode가 SAFETY_LOCKED_UNTIL_SEPARATE_APPROVAL_AND_FINAL_GATE다', () => {
    const view = buildTmsFastConnectionNaverProductLookupOneTimeLiveTestApprovalPacketView(
      buildTask406View(cases[0].input)
    );
    assert.equal(view.recommendedSafetyMode, 'SAFETY_LOCKED_UNTIL_SEPARATE_APPROVAL_AND_FINAL_GATE');
  });
});
