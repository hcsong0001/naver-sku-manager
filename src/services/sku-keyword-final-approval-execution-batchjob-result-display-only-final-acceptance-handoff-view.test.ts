...
test('sku-keyword-final-approval-execution-batchjob-result-display-only-final-acceptance-handoff-view.service', async (t) => {
  await t.test('should build batchjob execution result display only final acceptance handoff view correctly', () => {
    const view = buildNaverApiTokenFirstTestSeparateApprovalFinalHoldNonReleaseHandoffClosureFinalStatusSealConfirmationFinalReviewClosureStatusFinalClosureFinalStatusExecutionBatchJobResultDisplayOnlyFinalAcceptanceHandoffView(null);

    assert.equal(view.taskName, 'Task 185 - BatchJob Execution Result Display-Only Final Acceptance Handoff Screen Flow');
    assert.equal(view.panelTitle, 'BatchJob Execution Result Display-Only Final Acceptance Handoff');
    assert.equal(view.handoffStatus, 'BATCHJOB_RESULT_DISPLAY_ONLY_FINAL_ACCEPTANCE_HANDOFF_ACTIVE');

    assert.equal(view.isReadOnly, true);
    assert.equal(view.isDisplayOnlyFinalAcceptanceHandoff, true);
    ...
    assert.equal(view.previousExecutionBatchJobResultDisplayOnlyFinalAcceptanceSealCommit, 'c0db0ee');

    assert.equal(view.finalAcceptanceHandoffItems.length, 1);
    ...
    assert.ok(view.finalNotice.includes('final acceptance seal은 실행 허가가 아니며'));
