...
const SERVICE_FILE = resolve(
  __dirname,
  'sku-keyword-final-approval-execution-naver-api-token-first-test-separate-approval-final-hold-non-release-handoff-closure-final-status-seal-confirmation-final-review-closure-status-final-closure-non-release-seal-view.service.ts'
);
...
describe('Task 107 View Model 기본 필드', () => {
  ...
  it('statusLabel이 FINAL CLOSURE NON-RELEASE SEAL을 포함함', () => {
    assert.ok(view.statusLabel.includes('FINAL CLOSURE NON-RELEASE SEAL'));
  });
  ...
  it('finalNotice가 Task 107 이후에도 전환 불가를 표현함', () => {
    assert.ok(view.finalNotice.includes('Task 107'));
...
describe('Task 범위와 이전 커밋 참조', () => {
  ...
  it('taskRangeLabel이 Task 41~106 흐름을 표현함', () => {
    assert.ok(view.taskRangeLabel.includes('Task 41'));
    assert.ok(view.taskRangeLabel.includes('106'));
  });

  it('previousFinalClosureBoundaryCommit이 Task 106 기준 커밋임', () => {
    assert.strictEqual(view.previousFinalClosureBoundaryCommit, 'cec9cd5');
  });

  it('previousFinalClosureBoundaryLabel이 Task 106을 참조함', () => {
    assert.ok(view.previousFinalClosureBoundaryLabel.includes('Task 106'));
  });
});

describe('Final Closure Seal Summary', () => {
  const items = buildView().finalClosureSealSummaryItems;
...
describe('Final Closure Boundary Non-Release Seal', () => {
  const items = buildView().finalClosureBoundaryNonReleaseSealItems;
...
describe('Required Before Any Future Transition', () => {
  ...
  it('Task 41~107 증거를 포함함', () => {
    assert.ok(items.some((item) => item.requiredEvidence.includes('Task 41~107')));
  });
...
describe('순수 함수와 참조 격리', () => {
  it('여러 호출이 동일한 핵심 상태를 반환함', () => {
    ...
    assert.strictEqual(first.previousFinalClosureBoundaryCommit, second.previousFinalClosureBoundaryCommit);
  });
  ...
  it('배열 참조가 호출마다 분리됨', () => {
    assert.notStrictEqual(buildView().finalClosureSealSummaryItems, buildView().finalClosureSealSummaryItems);
