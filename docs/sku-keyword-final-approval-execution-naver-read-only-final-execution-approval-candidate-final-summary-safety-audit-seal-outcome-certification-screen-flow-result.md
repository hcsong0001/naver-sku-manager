# Task 318 - Naver Read-Only Final Execution Approval Candidate Final Summary Safety Audit Seal Outcome Certification Screen Flow

## 구현 목적

Task 317 후보 최종 요약 Safety Audit Seal 결과를 기반으로, Safety Audit Seal 결과를 read-only로 인증하는 패널을 추가합니다.

이 화면은 실제 승인, 실제 실행, 상품 변경 승인이 아닙니다.

## 상태 매핑 규칙

Task 317 `candidateFinalSummarySafetyAuditSealStatus`를 아래처럼 1:1로 매핑합니다.
...
Task 317의 `safetySealItems`를 그대로 재사용합니다.
...
- 전체 Safety Audit Seal 결과 인증 상태 배지
- READY / PARTIAL / BLOCKED / EMPTY / LOCKED 인증 카드
- 인증 대상 흐름 테이블
- read-only 완료 상태 유지 여부
- safeDisplayFields / excludedFields 유지 인증 여부
- 실행 / mutation / API 호출 잠금 유지 인증 여부
- Task 319 별도 승인 안내 문구
...
Task 319는 사용자 별도 명시 승인 후에만 진행합니다.
