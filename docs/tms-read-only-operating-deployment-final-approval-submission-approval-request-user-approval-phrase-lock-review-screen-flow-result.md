...
Task 396의 `User Approval Phrase Preparation Boundary Outcome Certification` 결과를 기반으로,
사용자 승인 문구 Lock이 계속 유지되는지 read-only로 검토하는 화면을 추가한다.
...
- 시작 기준: `HEAD = origin/main = bfd23ded91208d8f076a67750fde5b77960b19ca`
- 워킹 트리: clean
...
## 14개 User Approval Phrase Lock Review 그룹

1. `USER_APPROVAL_PHRASE_LOCK_REVIEW_READINESS`
2. `USER_APPROVAL_PHRASE_PREPARATION_BOUNDARY_OUTCOME_CERTIFICATION_REFERENCE_LOCK_REVIEW`
3. `USER_APPROVAL_PHRASE_REQUIRED_LOCK_REVIEW`
4. `USER_APPROVAL_PHRASE_EXAMPLE_DISPLAY_LOCK_REVIEW`
5. `USER_APPROVAL_PHRASE_NON_INPUT_LOCK_REVIEW`
6. `USER_APPROVAL_PHRASE_NON_SUBMISSION_LOCK_REVIEW`
7. `USER_APPROVAL_ACCEPTANCE_NON_GRANT_LOCK_REVIEW`
8. `EXPLICIT_APPROVAL_REQUEST_CREATION_NON_EXECUTION_LOCK_REVIEW`
9. `EXPLICIT_APPROVAL_REQUEST_SUBMISSION_NON_EXECUTION_LOCK_REVIEW`
...
모든 그룹은 Task 396 결과를 read-only로 참조하며, 실제 승인 문구 입력/제출 또는 실제 승인 요청 생성/제출로 해석되지 않도록 고정한다.

## 승인 문구 Lock Review는 read-only
...
권장 승인 문구 예시는 아래 값으로 read-only 검토만 제공한다.

- `TMS 운영 배포 최종 승인 요청 제출을 별도로 승인합니다.`

이번 화면은 사용자 승인 문구 Lock Review 화면이다.
...
- `app/api/sku-matching/draft-batch/[jobId]/route.ts`
  - Task 396 ViewModel을 입력으로 Task 397 ViewModel 생성
  - GET 응답에 `tmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseLockReviewView` 추가
  - POST API는 추가하지 않음
- `app/dashboard/sku-keyword-draft-batches/[jobId]/page.tsx`
  - Task 397 타입 필드 추가
  - Task 396 바로 아래, Task 332 바로 위에 Task 397 패널 추가
  - 상태, 결정값, 14개 그룹 요약, 승인 문구 예시 read-only 검토 안내, 안전 모드를 표시
...
## User Approval Phrase Lock Review는 실제 승인/제출이 아님

- 현재 화면은 read-only 사용자 승인 문구 Lock 검토 전용이다.
...
- `Task 398 - 운영 배포 최종 승인 제출 User Approval Phrase Lock 결과 인증 화면`
