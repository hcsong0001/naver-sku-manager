...
Task 397의 `User Approval Phrase Lock Review` 결과를 기반으로,
사용자 승인 문구 Lock 결과가 인증 가능한 상태인지 read-only로 확정 표시하는 결과 인증 화면을 추가한다.
...
- 시작 기준: `HEAD = origin/main = f5b2a5331121b0046625eb5a38417044239355f6`
- 워킹 트리: clean
...
## 14개 User Approval Phrase Lock Outcome Certification 그룹

1. `USER_APPROVAL_PHRASE_LOCK_OUTCOME_CERTIFICATION_READINESS`
2. `USER_APPROVAL_PHRASE_LOCK_REVIEW_REFERENCE_OUTCOME_CERTIFICATION`
3. `USER_APPROVAL_PHRASE_REQUIRED_LOCK_OUTCOME_CERTIFICATION`
4. `USER_APPROVAL_PHRASE_EXAMPLE_DISPLAY_LOCK_OUTCOME_CERTIFICATION`
5. `USER_APPROVAL_PHRASE_NON_INPUT_LOCK_OUTCOME_CERTIFICATION`
6. `USER_APPROVAL_PHRASE_NON_SUBMISSION_LOCK_OUTCOME_CERTIFICATION`
7. `USER_APPROVAL_ACCEPTANCE_NON_GRANT_LOCK_OUTCOME_CERTIFICATION`
8. `EXPLICIT_APPROVAL_REQUEST_CREATION_NON_EXECUTION_LOCK_OUTCOME_CERTIFICATION`
9. `EXPLICIT_APPROVAL_REQUEST_SUBMISSION_NON_EXECUTION_LOCK_OUTCOME_CERTIFICATION`
...
모든 그룹은 Task 397 결과를 read-only로 인증하며, 실제 승인 문구 입력/제출 또는 실제 승인 요청 생성/제출로 해석되지 않도록 고정한다.

## 승인 문구 Lock 결과 인증은 read-only
...
이번 화면은 사용자 승인 문구 Lock 결과 인증 화면이다.
...
- `app/api/sku-matching/draft-batch/[jobId]/route.ts`
  - Task 397 ViewModel을 입력으로 Task 398 ViewModel 생성
  - GET 응답에 `tmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseLockOutcomeCertificationView` 추가
  - POST API는 추가하지 않음
- `app/dashboard/sku-keyword-draft-batches/[jobId]/page.tsx`
  - Task 398 타입 필드 추가
  - Task 397 바로 아래, Task 332 바로 위에 Task 398 패널 추가
...
## User Approval Phrase Lock 결과 인증은 실제 승인/제출이 아님

- 현재 화면은 read-only 사용자 승인 문구 Lock 결과 인증 전용이다.
