...
Task 399의 `User Approval Phrase Explicit Input Boundary` 결과를 기반으로,
사용자 승인 문구 명시 입력 직전 경계 결과가 인증 가능한 상태인지 read-only로 확정 표시하는 결과 인증 화면을 추가한다.
...
- 시작 기준: `HEAD = origin/main = 5a2bf30c1b929d4e3195ccb27d0f2a5e776c4e25`
- 워킹 트리: clean
...
## 14개 User Approval Phrase Explicit Input Boundary Outcome Certification 그룹

1. `USER_APPROVAL_PHRASE_EXPLICIT_INPUT_BOUNDARY_OUTCOME_CERTIFICATION_READINESS`
2. `USER_APPROVAL_PHRASE_EXPLICIT_INPUT_BOUNDARY_REFERENCE_OUTCOME_CERTIFICATION`
3. `USER_APPROVAL_PHRASE_REQUIRED_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
4. `USER_APPROVAL_PHRASE_EXAMPLE_DISPLAY_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
5. `USER_APPROVAL_PHRASE_INPUT_UI_NON_ADDITION_OUTCOME_CERTIFICATION`
6. `USER_APPROVAL_PHRASE_INPUT_NON_EXECUTION_OUTCOME_CERTIFICATION`
7. `USER_APPROVAL_PHRASE_SUBMISSION_NON_EXECUTION_OUTCOME_CERTIFICATION`
8. `USER_APPROVAL_ACCEPTANCE_NON_GRANT_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
9. `EXPLICIT_APPROVAL_REQUEST_CREATION_NON_EXECUTION_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
10. `EXPLICIT_APPROVAL_REQUEST_SUBMISSION_NON_EXECUTION_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
11. `FINAL_APPROVAL_SUBMISSION_NON_EXECUTION_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
12. `DEPLOYMENT_OPERATING_TRANSITION_NON_EXECUTION_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
13. `OPERATING_DB_RUNTIME_WORKER_QUEUE_ADAPTER_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`
14. `API_SECRET_UI_ACTION_POST_EXPLICIT_INPUT_OUTCOME_CERTIFICATION`

모든 그룹은 Task 399 결과를 read-only로 인증하며, 실제 승인 문구 입력 또는 실제 승인 요청 생성/제출로 해석되지 않도록 고정한다.

## 승인 문구 Explicit Input Boundary 결과 인증은 read-only
...
권장 승인 문구 예시는 아래 값으로 read-only 결과 인증만 제공한다.

- `TMS 운영 배포 최종 승인 요청 제출을 별도로 승인합니다.`

이번 화면은 사용자 승인 문구 Explicit Input Boundary 결과 인증 화면이다.
...
- `app/api/sku-matching/draft-batch/[jobId]/route.ts`
  - Task 399 ViewModel을 입력으로 Task 400 ViewModel 생성
  - GET 응답에 `tmsReadOnlyOperatingDeploymentFinalApprovalSubmissionApprovalRequestUserApprovalPhraseExplicitInputBoundaryOutcomeCertificationView` 추가
  - POST API는 추가하지 않음
- `app/dashboard/sku-keyword-draft-batches/[jobId]/page.tsx`
  - Task 400 타입 필드 추가
  - Task 399 바로 아래, Task 332 바로 위에 Task 400 패널 추가
  - 상태, 결정값, 14개 그룹 요약, 승인 문구 예시 read-only 결과 인증 안내, 안전 모드를 표시
...
## User Approval Phrase Explicit Input Boundary 결과 인증은 실제 승인/제출이 아님

- 현재 화면은 read-only 사용자 승인 문구 명시 입력 경계 결과 인증 전용이다.
...
- `Task 401 - 운영 배포 최종 승인 제출 User Approval Phrase Final Pre-Input Lock Review 화면`
