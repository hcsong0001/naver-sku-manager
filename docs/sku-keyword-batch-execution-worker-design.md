...
상세 입력 필드, 재검증, 변환 실패와 감사 정책은 [SKU Keyword Matching Batch payload 변환 설계](./sku-keyword-batch-payload-transform-design.md)를 따른다.
...
실행 직전 승인 범위, 승인 artifact, `dryRun` 처리 선택지와 Worker 권한 경계는 [SKU Keyword Matching Batch 실행 직전 최종 승인 설계](./sku-keyword-batch-final-approval-design.md)를 따른다.
승인 artifact를 실제로 보존하기 위한 기존 모델 수용 가능성과 후보 schema는 [SKU Keyword Matching 최종 승인 artifact 스키마 필요성 검토](./sku-keyword-batch-final-approval-schema-review.md)를 따른다.
...
1. [payload 변환 설계](./sku-keyword-batch-payload-transform-design.md)를 기준으로 공식 API operation과 transformer 타입 검토
