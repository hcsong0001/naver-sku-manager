...
async function run() {
  ...
  try {
    ...
    const executingItems = items.filter(i => i.status === 'EXECUTING');
    ...
    console.log('[Restore] BatchJobItem EXECUTING count:', executingItems.length);

    // If already in pre-run state, skip restore
    if (currentJobStatus === 'APPROVED' && executingItems.length === 0) {
    ...
    if (currentJobStatus !== 'EXECUTING') {
      console.error(
        `[Restore ABORT] BatchJob.status is "${currentJobStatus}" — only EXECUTING state can be restored`
      );
    ...
    console.log('[Restore] Expected restore: BatchJob EXECUTING → APPROVED');
    console.log('[Restore] Expected restore: BatchJobItem EXECUTING × ', executingItems.length, '→ READY');

    // ── 5. Perform restore in a transaction ──
    // WHERE conditions include fixture ID to prevent accidental wider updates
    await prisma.$transaction(async tx => {
      // Restore BatchJob status
      ...
      if (jobRestoreResult.count === 0) {
        throw new Error(
          `BatchJob restore failed: id=${batchJobId} was not in EXECUTING state — 0 rows affected`
        );
      }
      ...
      if (itemRestoreResult.count === 0) {
        throw new Error(
          `BatchJobItem restore failed: batchJobId=${batchJobId} had no EXECUTING items — 0 rows affected`
        );
      }
    });
